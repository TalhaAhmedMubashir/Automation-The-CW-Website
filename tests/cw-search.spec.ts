...
test.describe("Search Test", () => {
    ...
    const imagesrc = '//li[1][contains(@class, "matched")]//a//img[contains(@class, "lazyloaded")]';

    const iframeLocator = "//iframe[@title='Advertisement' and not(contains(@style, 'display: none'))]";
    ...
    test("Search Function Test", async ({ page }) => {
        ...
        await test.step("Point 2 : Verify Placeholder value", async () => {
            ...
            const searchInput = await page.locator(searchbar);
            ...
            expect.soft(placeholderValue, "Search bar placeholder value does not matched. Test failed.").toBe('Search for a series or movie...');
        })
        ...
        await test.step("Point 4 : Type 2 letter result in thumbnails", async () => {
            ...
            expect.soft(await page.locator(issearchcountVisible).isVisible(), "Result in thumbnails is not being displayed on Two letter. Test Failed").toBeTruthy()
        })
        ...
        await test.step("Point 9 : Search and Open Show", async () => {
            ...
            expect.soft(await (page.locator(searchresulthastitle)).first().isVisible(), `Title "${searchbardata}" does not matched.`).toBeTruthy();
            if (await (page.locator(searchresulthastitle)).first().isVisible()) {
                expect.soft(await IsImagebroken(page, imagesrc), "Image link is broken").toBeTruthy();
            }

            //match serices/movies/episode
            expect.soft(await (page.locator(searchresulthastype)).isVisible(), `Type "serices/movies/episode" does not matched.`).toBeTruthy();

            //Open and match url by clicking serices/movies/episode
            expect.soft(await isDetailPageOpened(page, searchresulthastype, matchurl), `Failed to open detail page. Expected URL: ${matchurl}`).toBeTruthy();
            await page.goBack()

            await page.waitForLoadState()
            //Open and match url by clicking on image
            expect.soft(await isDetailPageOpened(page, imagesrc, matchurl), `Failed to open detail page. Expected URL: ${matchurl}`).toBeTruthy();
        });
        ...
        await test.step("Point 11 : Search episode and Check image", async () => {
            ...
            const searchresulthastitle = `//div[@class="titles" and contains(., "${searchbardata}")]`;
            const similaritempath_ondetailpage = '//div[@id="touchcarousel_1"]//ul//li';
            ...
            if (await (page.locator(searchresulthastitle)).first().isVisible()) {
                ...
                await page.waitForTimeout(2000)
                const isDetailPageDisplaying = await isDetailPageOpened(page, imagesrc, matchurl)
                expect.soft(isDetailPageDisplaying, `Failed to open detail page. Expected URL: ${matchurl}`).toBeTruthy();

                if (isDetailPageDisplaying) {
                    let iframeVisible = await isiframeVisible(page, iframeLocator)
                    expect.soft(iframeVisible, `Video Player is not visible.`).toBeTruthy();

                    if (await isiframeVisible(page, iframeLocator)) {
                        const hasmorevideos = await isSimilarEpisodesVisible(page, similaritempath_ondetailpage)
                        expect.soft(hasmorevideos, `Does not have more episodes.`).toBeTruthy();
    ...
    test("Remove character from search", async ({ page }) => {
        ...
        await test.step("Point 7 : Enter Random Characters in Search", async () => {
            ...
            expect.soft(await page.locator(noresultfoundpath).isVisible(), "Randon character in search field, test failed.").toBeTruthy()
        })
        ...
        await test.step("Point 8 : Remove characters and identify the count change in search result.", async () => {
            ...
            for (let i = searchbardata.length - 1; i > 0; i--) {
                const rawText = await page.locator(searchcountpath).first().textContent();
            ...
            // Verify the results
            expect.soft(differenceOccurance !== 0, "Remove characters and identify the count change in search result. Test Failed.").toBeTruthy();
        })
...
async function isiframeVisible(page: Page, iframelocator: string) {
    ...
    const adFrame = await page.locator(iframelocator).contentFrame();
    await page.waitForTimeout(5000)

    // Check if the iframe is visible
    return await adFrame ? true : false;
}
...
async function isDetailPageOpened(page: Page, sourcetoclick: string, matched_substring_url: string) {
